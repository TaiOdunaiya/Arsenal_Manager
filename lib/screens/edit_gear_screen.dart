...
class EditGearScreen extends ConsumerStatefulWidget {
  final GearItem item;
  const EditGearScreen({super.key, required this.item});

  @override
  ConsumerState<EditGearScreen> createState() => _EditGearScreenState();
}

class _EditGearScreenState extends ConsumerState<EditGearScreen> {
  ...
  Future<void> _submit() async {
    ...
    final success = await ref.read(arsenalProvider.notifier).updateGear(
          id: widget.item.id,
  ...
  Future<void> _delete() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Remove ${widget.item.name} from the arsenal?',
    ...
    if (confirm == true && mounted) {
      setState(() => _submitting = true);
      await ref.read(arsenalProvider.notifier).deleteGear(widget.item.id);
